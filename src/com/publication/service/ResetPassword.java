...
public class ResetPassword extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(newPassword.isEmpty() || newPasswordAgain.isEmpty()){
			out.println(returnScript("Empty fields found.", "index.jsp"));
			return;
		}
		if(!newPassword.equals(newPasswordAgain)){
			out.println(returnScript("Passwords do not match.", "index.jsp"));
			return;
		}else{
			if(dao.finishForgetPassword(request.getParameter("id"), newPassword)){
				out.println(returnScript("Password Reset Successfull! Login again now.", "index.jsp"));
				return;
			}else{
				out.println(returnScript("Password cannot be changed, try again later.", "index.jsp"));
				return;
			}
